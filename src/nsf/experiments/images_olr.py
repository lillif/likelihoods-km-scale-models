...
import src.nsf.nn as nn_
import src.nsf.optim as optim
import src.nsf.utils as utils
from src.nsf.experiments import autils
from src.nsf.experiments.autils import Conv2dSameSize
from src.nsf.nde import distributions, flows, transforms
from src.nsf.olr_data.transforms import InverseMinMaxNormaliseTransform
...
def create_transform(
            ...
            transform_level = transforms.CompositeTransform(
                ...
                + [
                    create_transform_step(
                        c, level_hidden_channels, **transform_step_kwargs
                    )
    ...
    if (
        preprocessing == "olr"
    ):  # preprocessing flag remains from nsf codebase, not used for OLR data
        # OLR data is already in the range [0, 1]
        pass  # No preprocessing
    else:
        raise RuntimeError(f"Unknown preprocessing type: {preprocessing}")
...
def create_flow(
    ...
    transform = create_transform(
        c, h, w, **create_transform_kwargs, transform_step_kwargs=transform_step_kwargs
    )
...
class Flow(LightningModule):
    """
    A PyTorch Lightning wrapper for training normalizing flows on image data.
    """
    ...
    def __init__(
        ...
        temperatures: list = [1.0],  # temperatures for sampling
        warmup_fraction: float = 0.0,
        sample_interval: int = 1,
        samples_per_temperature: int = 4,
    ):
        super().__init__()
        ...
        self.samples_per_temperature = samples_per_temperature

        self.olr_plotting_transform = InverseMinMaxNormaliseTransform()
    ...
    def training_step(self, batch, batch_idx):
        ...
        # check if x is empty
        if x.numel() == 0:
            self.log("train/loss_skipped_batches", 1, prog_bar=True)
            logger.debug(f"batch index {batch_idx} is {batch}")
            logger.warning("Skipping training step due to empty batch.")
            return None
        try:
            ...
            logger.error(
                f"Error during training step with batch index {batch_idx}: {e}"
            )
            return None
    ...
    def validation_step(self, batch, batch_idx):
        x = batch[self.data_key]
        if x.numel() == 0:
            self.log("val/loss_skipped_batches", 1, prog_bar=True)
            logger.warning("Skipping validation step due to empty batch.")
            return None
        try:
            log_prob = self.flow.log_prob(x)
            ...
            logger.error(
                f"Error during validation step with batch index {batch_idx}: {e}"
            )
            return None
    ...
    def test_step(self, batch, batch_idx):
        x = batch[self.data_key]
        if x.numel() == 0:
            self.log("test/loss_skipped_batches", 1, prog_bar=True)
            logger.warning("Skipping test step due to empty batch.")
            return None
        log_prob = self.flow.log_prob(x)
        test_loss = -autils.nats_to_bits_per_dim(log_prob.mean(), *self.dataset_dims)
    ...
    def on_validation_epoch_end(self):
        ...
        with torch.no_grad():
            fig, axs = plt.subplots(
                1, len(self.temperatures), figsize=(4 * len(self.temperatures), 4)
            )

            fig_olr, axs_olr = plt.subplots(
                1, len(self.temperatures), figsize=(4 * len(self.temperatures), 4)
            )

            if len(self.temperatures) > 1:
                axs = axs.flat
                axs_olr = axs_olr.flat
            else:
                axs = [axs]
                axs_olr = [axs_olr]

            for temperature, ax, olr_ax in zip(self.temperatures, axs, axs_olr):
                device = next(self.parameters()).device
                noise = (
                    self.flow._distribution.sample(self.samples_per_temperature)
                    * temperature
                )
                noise = noise.to(device)
                samples, _ = self.flow._transform.inverse(noise)
                im_grid = utils.make_grid(
                    samples, nrow=int(self.samples_per_temperature**0.5)
                )
                autils.imshow(im_grid, ax)
                ax.set_title(f"T={temperature:.2f}")

                ## plot OLR samples
                olr_samples = self.olr_plotting_transform(
                    samples
                )  # from [0,1] to OLR range
                olr_im_grid = utils.make_grid(
                    olr_samples, nrow=int(self.samples_per_temperature**0.5)
                )
                autils.imshow(
                    olr_im_grid,
                    olr_ax,
                    vmin=self.olr_plotting_transform.min,
                    vmax=self.olr_plotting_transform.max,
                )
                olr_ax.set_title(f"T={temperature:.2f}")

            self.logger.experiment.log(
                {f"val/samples_{self.current_epoch}": wandb.Image(fig)}
            )

            self.logger.experiment.log(
                {f"val/olr_samples_{self.current_epoch}": wandb.Image(fig_olr)}
            )
