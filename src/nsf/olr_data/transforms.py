...
class HistogramMatchingTransform:
    def __init__(self, matcher_path: str, key: str = "image"):
        matcher = np.load(matcher_path)
        self.bin_edges = matcher["bin_edges"]
        self.lookup_table = matcher["lookup_table"]
        self.key = key

    def _match_histogram(self, data: np.ndarray) -> np.ndarray:
        """
        Map values of `data` using the precomputed histogram lookup_table.
        a source value in bin i should be mapped to lookup_table[i]
        """

        # digitize source values into bins
        idx = np.digitize(data.ravel(), self.bin_edges) - 1

        if np.any(idx < 0) or np.any(idx >= len(self.lookup_table)):
            logger.warning(
                f"some values are out of the histogram bin range; shouldn't happen because we used overall min and max for histogram range.\ndata min: {data.min()}, data max: {data.max()}, bin_edges min: {self.bin_edges.min()}, bin_edges max: {self.bin_edges.max()}"
            )

        idx = np.clip(idx, 0, len(self.lookup_table) - 1)  # avoid out-of-bounds

        # replace original pixel values with matched values
        matched_data = self.lookup_table[idx].reshape(data.shape)
        return matched_data

    def __call__(self, data_dict):
        data = data_dict[self.key]
        matched_data = self._match_histogram(data)
        data_dict[self.key] = matched_data
        return data_dict
...
### for plotting samples from normalising flows
class InverseMinMaxNormaliseTransform:
    def __init__(
        self,
        minmax_path: str = os.path.join(
            os.environ.get("DATAROOT"), "minmax_goes_first15dayseachmonth.json"
        ),
    ):
        with open(minmax_path) as f:
            minmax_dict = json.load(f)
