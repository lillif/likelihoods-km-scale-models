...
def load_ckpt_from_hydra_run(
    ...
    if len(ckpts) > 1 and ckpt_type == "best":
        ...
        try:
            ...
            best_ckpt = ckpts[-1]
    elif len(ckpts) > 1 and ckpt_type == "last":
        print(f"there are {len(ckpts)} checkpoints, attempting to use the last one")
        try:
            best_ckpt = glob(f"{hydra_run_path}/*/*last*")[0]
        except IndexError:  # if no "best" model exists
            print(f"could not load last model, attempting last (?) model instead")
            best_ckpt = ckpts[-1]
    else:
